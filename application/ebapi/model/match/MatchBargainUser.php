...
class MatchBargainUser extends ModelBasic
{
    ...
    public static function setBargain($bargainId = 0,$bargainUserUid = 0){
        if(!$bargainId || !$bargainUserUid || !MatchBargain::validBargain($bargainId) || self::be(['bargain_id'=>$bargainId,'uid'=>$bargainUserUid,'status'=>1,'is_del'=>0])) return false;
        $data['bargain_id'] = $bargainId;
        $data['uid'] = $bargainUserUid;
        $data['bargain_price_min'] = MatchBargain::where('id',$bargainId)->value('min_price');
        $data['bargain_price'] = MatchBargain::where('id',$bargainId)->value('price');
        $data['price'] = 0;
        $data['status'] = 1;
        $data['is_del'] = 0;
        $data['add_time'] = time();
    ...
    public static function isBargainUser($bargainId = 0,$bargainUserUid = 0){
        if(!$bargainId || !$bargainUserUid || !MatchBargain::validBargain($bargainId)) return false;
    ...
    public static function getBargainUserAll($bargainUserUid = 0,$offset = 0,$limit = 20){
       ...
       $model = $model->join('MatchBargain b','b.id=u.bargain_id');
    ...
    public static function editBargainUserStatus($uid){
        ...
        $bargainProduct = MatchBargain::validBargainNumber(); //TODO 获取正在开启的砍价产品编号
        $closeBargain = [];
    ...
    public static function getBargainUserStatusSuccess(){
        ...
        if($bargainUser) {
            ...
            foreach ($bargainUser as $k=>$v){
                $bargainUser[$k]['info'] = User::where('uid',$v['uid'])->value('nickname').'砍价成功了'.$v['bargain_price_min'].'砍到了'.MatchBargain::where('id',$v['bargain_id'])->value('title');
            }
        }
        else{
