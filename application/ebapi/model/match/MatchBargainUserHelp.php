<?php
namespace app\ebapi\model\match;
...
/**
 * 砍价帮砍Model
 * Class MatchBargainUser
 * @package app\ebapi\model\Match
 */
...
//        dump(MatchBargainUser::be(['bargain_id'=>$bargainId,'uid'=>$bargainUserUid,'status'=>1,'is_del'=>0]));
        ...
        $coverPrice = MatchBargainUser::getBargainUserDiffPriceFloat($bargainUserTableId);//TODO 用户可以砍掉的金额 好友砍价之前获取可以砍价金额
        $alreadyPrice= MatchBargainUser::getBargainUserPrice($bargainUserTableId);//TODO 用户已经砍掉的价格
        ...
        self::beginTrans();
        $res1 = MatchBargainUser::setBargainUserPrice($bargainUserTableId,$bargainUserData);
        $res2 = self::set($data);
        $res = $res1 && $res2;
        self::checkTrans($res);
        return $res;
    ...
    public static function getSurplusPrice($bargainId = 0,$bargainUserUid = 0){
        $bargainUserTableId = MatchBargainUser::getBargainUserTableId($bargainId,$bargainUserUid);// TODO 获取用户参与砍价表编号
        $coverPrice = MatchBargainUser::getBargainUserDiffPriceFloat($bargainUserTableId);//TODO 获取用户可以砍掉的金额  好友砍价之后获取砍价金额
    ...
    public static function getSurplusPricePercent($bargainId = 0,$bargainUserUid = 0){
        $bargainUserTableId = MatchBargainUser::getBargainUserTableId($bargainId,$bargainUserUid); // TODO  获取用户参与砍价表编号 计算进度条
        $coverPrice = MatchBargainUser::getBargainUserDiffPriceFloat($bargainUserTableId);//TODO 用户可以砍掉的金额 计算进度条
        $alreadyPrice = MatchBargainUser::getBargainUserPrice($bargainUserTableId);//TODO 用户已经砍掉的价格 计算进度条
