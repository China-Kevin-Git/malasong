 ...
 */
class MatchGoods extends AuthController
{
    ...
    {
        $match = Db::name("match_goods")->select();
    ...
    public function create(){
        ...
        $f[] = Form::input('goods_name','可选服务商品名');
        $f[] = Form::number('price','产品售价')->min(0)->col(8);
        $f[] = Form::number('market_price','产品市场价')->min(0)->col(8);
    ...
    public function save(Request $request){
        ...
        if(!$data['goods_name']) return Json::fail('请输入可选服务商品名');
        ...
        if(empty($data['market_price'])) return Json::fail('产品市场价');
        ...
        foreach ($label as $k=>$v){
            $data['match_id'] = $v;
            Db::name("match_goods")->insert($data);
        }
    ...
    public function edit($id){
        ...
        $article = Db::name("match_goods")->where(['service_id'=>$id])->find();
...
//        )->col(Form::col(50));
        ...
        $f[] = Form::input('goods_name','可选服务商品名',$article['goods_name']);
        $f[] = Form::number('price','产品售价',$article['price'])->min(0)->col(8);
        $f[] = Form::number('market_price','产品市场价',$article['market_price'])->min(0)->col(8);
    ...
    public function update(Request $request, $id)
    {
        ...
        unset($data["image"]);
        Db::name("match_goods")->where(['service_id'=>$id])->update($data);
    ...
    {
        $res = Db::name("match_goods")->where(['service_id'=>$id])->delete();
        if(!$res)
