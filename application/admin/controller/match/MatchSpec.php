 ...
 * 文章分类管理  控制器
 * */
class MatchSpec extends AuthController

{
    ...
    public function index(){
        ...
        $match_spec =  Db::name("match_spec")->select();

        $this->assign("list",$match_spec);
        return $this->fetch();
    }
    ...
    public function save(Request $request){
        ...
        $new_id = Db::name("match_spec")->insert(["name"=>$data['name']]);
    ...
    public function edit($id){
        ...
        $article= Db::name("match_spec")->where(["spec_id"=>$id])->find();
    ...
    public function update(Request $request, $id)
    {
        ...
        if(!$data['name']) return Json::fail('请输入分类名称');
        Db::name("match_spec")->where(["spec_id"=>$id])->update(["name"=>$data["name"]]);
    ...
    {
        $res = Db::name("match_spec")->where(["spec_id"=>$id])->delete();
        if(!$res)
