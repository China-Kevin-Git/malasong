                    ...
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" name="match_name" lay-verify="match_name" style="width: 100%" autocomplete="off" placeholder="请输入产品名称,编号" class="layui-input">
                            </div>
                        </div>
                        <div class="layui-inline">
...
<script>
    ...
    layList.search('export',function(where){
        location.href=layList.U({c:'umps.match_bargain',a:'get_bargain_list',q:{
                match_name:where.match_name,
                status:where.status,
                export:1,
            }});
    })
    layList.switch('status',function (odj,value,name){
        if(odj.elem.checked==true){
            layList.baseGet(layList.Url({c:'umps.match_bargain',a:'set_bargain_status',p:{status:1,id:value}}),function (res) {
                layList.msg(res.msg);
            });
        }else{
            layList.baseGet(layList.Url({c:'umps.match_bargain',a:'set_bargain_status',p:{status:0,id:value}}),function (res) {
                layList.msg(res.msg);
            });
        }
    });
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'delstor':
                var url=layList.U({c:'umps.match_bargain',a:'delete',q:{id:data.id}});
